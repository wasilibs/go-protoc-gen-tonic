...
go-protoc-gen-tonic is a distribution of the code generator of [tonic][1].
It does not actually reimplement any functionality of tonic in Go, instead compiling it
...
Precompiled binaries are available in the [releases](https://github.com/wasilibs/go-protoc-gen-tonic/releases).
Alternatively, install the plugin using `go install`.

```bash
$ go install github.com/wasilibs/go-protoc-gen-tonic/cmd/protoc-gen-tonic@latest
```

As long as `$GOPATH/bin`, e.g. `~/go/bin` is on the `PATH`, you can use it with protoc as normal.

```bash
$ protoc --tonic_out=out/rust -Iprotos protos/helloworld.proto
```
...
plugins:
  - local:
      - go
      - run
      - github.com/wasilibs/go-protoc-gen-tonic/cmd/protoc-gen-tonic@latest
    out: out/rust
```
